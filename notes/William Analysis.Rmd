...
```{r MC-bin-bin-mix, include=FALSE, cache=TRUE}
# , dependson=c("boilerplate")}
...
for(j in seq_along(all_Ns)){
  ...
  for(i in 1:num_reps){
    print(paste0("n = ", n, ", i = ", i, " of ", num_reps))
  ...
  # Store output for current value of n
  all_a_hats[[j]] = some_a_hats
  all_theta_hats[[j]] = some_theta_hats
  all_b_hats[[j]] = some_b_hats
  all_gamma_hats[[j]] = some_gamma_hats
  
  all_M_covs[[j]] = some_M_covs
  all_Y_covs[[j]] = some_Y_covs
...
# Save output
# save(all_a_hats, all_theta_hats, all_b_hats, all_gamma_hats, all_M_covs, all_Y_covs, all_med_hats, all_med_SEs, file = "William-Analysis_cache/MC-bin-bin-mix.RData")
# load("William-Analysis_cache/MC-bin-bin-mix.RData")
```

We now process the output from our simulation and summarize the results in a table. We give the mean and median of the estimated standard errors, as well as their relative errors (in $\%$) as estimates of the empirical standard error. See Table \ref{tab:results_bin_bin_fix}.
...
# Combine into a table
# results = data.frame(Empirical = emp_SEs,
#                      Mean = mean_delta_SEs, Median = median_delta_SEs,
#                      "Percent_Err_Mean" = 100*(mean_delta_SEs - emp_SEs)/emp_SEs,
#                      "Percent_Err_Median" = 100*(median_delta_SEs - emp_SEs)/emp_SEs)
results = data.frame(n = all_Ns, Empirical = emp_SEs,
...
```{r results="asis", echo=FALSE}
## Print table
kbl(results, format = "latex", booktabs = T, caption = "Summary of SE estimates for mediation effect under continuous response, continuous mediator, fixed-effects model \\label{tab:results_bin_bin_fix}")
```



I don't really understand the results I'm getting here. It should help if I can compare my estimates of the mediation effect with the true value.

```{r}
load("../data/Exact_Asymptotics/Bin-Bin-Mix_Results.RData")

true_med = Phi_of_xi(a, theta, b, gamma, x_pred, W_pred)

load("William-Analysis_cache/latex/unnamed-chunk-37_44bd8d1152289e689fbff9185d17042c.RData", ex <- new.env())
ls.str(ex)
```


Investigate normality of estimators
```{r}
library(ggplot2)
library(magrittr)

med_hats_to_plot = unlist(data_med_hats)
Ns_to_plot = rep(all_Ns, times = sapply(data_med_hats, length))
data_med_hat_to_plot = data.frame(med_hat = med_hats_to_plot, N = Ns_to_plot)
...
all_norm_p_vals = c()
for(n in all_Ns){
  p_val = shapiro.test(data_med_hat_to_plot$med_hat[data_med_hat_to_plot$N == n])$p.value
